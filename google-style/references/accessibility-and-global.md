...
Gendered language has two sources, and both are avoidable. Pronouns come first: don't use `he`, `him`, `his`, `she`, or `her` as a gender-neutral pronoun, and don't reach for `he/she` or `(s)he` either. The singular `they` is the form. Role nouns come second, and `man-hours`, `mankind`, and `manpower` all have plain replacements in `person-hours`, `humanity`, and `workforce`. The `gendered` rule covers the punctuational pair forms, `he/she`, `s/he`, `his/her`, and `he or she`, together with the gendered role nouns, and reports each of them as an error. A bare `he` or `her` standing in as a gender-neutral pronoun is past what a pattern can judge, so that one stays with the writer.
