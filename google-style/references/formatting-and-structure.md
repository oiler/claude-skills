...
A space sits between a number and its unit, so `64 GB` rather than `64GB`, and the `units` rule reports a number glued to a unit abbreviation as an error. The guide asks for that space to be nonbreaking and writes it as the entity `&nbsp;` in both its HTML and its Markdown examples, which governs how the rendered line wraps rather than anything the checker can see: the rule enforces only that a space is there, and reads `U+00A0` and a plain space alike. Type a plain space in Markdown source, and reach for `&nbsp;` only where you control the rendered output and a line break between the number and the unit would hurt. Three cases take no space at all: money, percent, and degrees of an angle. Temperature is the split case: a space sits between the number and the degree symbol, and nothing sits between the degree symbol and the scale letter, giving `50 °C`. Kelvin drops the degree symbol and keeps the space, giving `300 K`.
