...
Running it twice is safe. The script reports the commit it already made, rather than making a second one, when all three of these hold: it found nothing to commit; the working tree holds no change at all beyond the acceptance command's leftovers; and a commit since the dispatch base carries a `Task <n>:` subject. Then it prints that commit's own sha — not HEAD, which anything may have moved since — with `"already": true`, and exits `0`, so a compaction between this step and step 6 costs nothing.

If any of the three fails it exits `2`, with `orko: nothing changed in code`, or with `orko: nothing to stage in code` when the task named no stageable path at all. Either is a stop to report. That is the point of the second condition: an uncommitted fix still sitting in the tree, in a file the task never listed, is never read as a delivery that landed.
...
Name the failure concretely: the finding, and for `diff-outside-allowlist` the paths. Dispatch the stdout verbatim as in step 2 and wait as in step 3. A failed delivery leaves the tree exactly as Codex left it, which is what `--resume` continues from: do not clean it up, do not commit it, and do not write the fix yourself. A second delivery failure is an escalation. A re-dispatch after step 6, on a task already committed, lands a second commit: pass `--message "<what the fix was>"`, which becomes the tail of the same `Task <n>:` subject.
