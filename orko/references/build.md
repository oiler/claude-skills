...
- **`escalated`** is an *additional* line, written immediately after that step's `complete` line, never instead of it. Recording only `escalated` pins `next_step` at that step forever and the resume re-runs it.
...
| `check delivery` fails twice on one task | `record decision` or `record adr` per the routing rule, `ledger 5.<n> escalated`, stop. |
