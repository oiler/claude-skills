...
`preflight` at step 5 confirms the spec is `accepted` and re-records the hash of every record the ledger names, because the gate asks a human to edit the spec and both reviews, so none of those edits is reported as tampering on the next write. It re-records only on a preflight that found nothing, and only from the committed content, so an uncommitted hand edit keeps tripping the guard. Report the `check spec` output, which the workspace `AGENTS.md` asks for before implementation begins.
...
| A human edited a record orko wrote | `record` refuses to overwrite a file whose hash differs from the last committed one. Read the change, report it, and continue from the human's version. The gate's expected edit is exempt because a clean `preflight` re-records it from the commit. |
