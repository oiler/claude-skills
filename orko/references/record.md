...
| Implementation | commits on `orko/<slug>` citing `SPEC-NNN` and `PLAN-NNN`; testing rows written by the implementer | `code/` | n/a | none | `uv run ${CLAUDE_SKILL_DIR}/scripts/orko.py commit code --slug <slug> --message "<subject>"`, or `--task <n>` for a Codex delivery |
...
So the gate asks a human for four fields in total: `status: accepted` and `approved_at` on the spec, `approved_by` on the spec review, and `approved_by` on the plan review. Name all four when you stop. `preflight` at step 5 confirms the spec is `accepted` and then re-records the hash of every record the ledger names that HEAD holds, from a working tree the dirty checks just passed, so none of those four edits is reported as tampering on the next write. A preflight with findings re-records nothing, and a record no commit holds gets no floor: `preflight` says so on stderr, and it stays unguarded until it is committed.
...
`commit docs` stages the run's own record paths in `docs/` plus every `touched` path the `record` commands logged there: `STATUS.md`, the version README, the changelog. `commit code` does the same for `code/`. Add an extra repo-relative path with `--path` (repeatable) when a run wrote something outside the record, such as `docs/adr/` files in the code repository. Code an implementer wrote is the implementer's own commit, not this one: `commit code` has nothing recorded to stage for it, and it refuses with exit `2` rather than sweeping the tree. Run it only when the run recorded something in `code/`, or name the files yourself with `--path`. Two flags add to that set and nothing else does: `--path`, and `--task <n>` on `commit code`, which stages one Codex delivery — the task's `**Files:**` paths plus `docs/testing/README.md` — under a `Task <n>:` subject, per [codex.md](codex.md). The commit is scoped to the staged paths, so a file the user had already staged in that repository stays staged rather than riding along under orko's message.

The message the script builds is your `--message` subject — or, under `--task <n>`, `Task <n>:` ahead of your message, or of the task's own title when you pass none — then a `Refs:` line naming every ID the run has minted, then the attribution trailers recorded at `init --trailer` (repeatable), verbatim. After the commit, the script records a `hash <path> <sha256>` ledger line for every file it staged.

Those hash lines are the overwrite guard. `record` refuses to write a file when all three hold: a hash is recorded for that path, the file exists, and its current sha differs from the recorded one. That means a human edited a record orko wrote. Read the change, report it, and continue from the human's version rather than overwriting it. The gate's expected edits are exempt: a clean `preflight` at step 5 re-records the hash of every committed record the ledger names once it confirms `status: accepted`, so neither the spec's acceptance nor an `approved_by` on either review is reported as tampering on the next write. Two things stay unexempt. A record no commit holds gets no floor at all, and `preflight` says so on stderr, because a floor is a committed hash and there is no commit to take one from; a staged record is not a committed one, and it stays unguarded until it is committed. And an uncommitted edit to a tracked record makes `preflight` itself exit `1` on `docs-dirty`, before it re-records anything.
