...
**Only the hook stubs step aside — the translation stubs stay.** Brain Monkey ships no `__()` or `esc_html__()`: `inc/wp-helper-functions.php` covers `__return_*`, `trailingslashit`, `absint`, `is_wp_error` and friends, and translation functions are opt-in behind `Monkey\Functions\stubTranslationFunctions()`. After the handoff runs, `add_action`/`add_filter` resolve into `vendor/brain/monkey/inc/wp-hook-functions.php` while `__`/`esc_html__` still resolve into your `tests/bootstrap.php` — permanently, unless you call `stubTranslationFunctions()` yourself. Do not delete the translation stubs on the assumption that Brain Monkey replaced them; that is a fatal undefined-function error the moment any code under test calls `__()`.
