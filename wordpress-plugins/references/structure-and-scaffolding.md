...
That is the whole upgrade — the emitted bootstrap already hands the hook definitions over. It calls `\Brain\Monkey\setUp()` right after the autoloader whenever `Brain\Monkey\setUp` exists, so Brain Monkey defines `add_action()` and `add_filter()` first and those two `function_exists()` guards step aside on their own. Tests move to `Monkey\Actions\expectAdded( 'init' )` style assertions and are otherwise ordinary PHPUnit.

**Only the hook stubs step aside — the translation stubs stay.** Brain Monkey ships no `__()` or `esc_html__()`; `inc/wp-helper-functions.php` covers `__return_*`, `trailingslashit`, `absint`, `is_wp_error` and friends. After the handoff runs, `add_action`/`add_filter` resolve into `vendor/brain/monkey/inc/wp-hook-functions.php` while `__`/`esc_html__` still resolve into your `tests/bootstrap.php`, and they stay there: the handoff loads Patchwork partway through the bootstrap, which counts that whole file as included before Patchwork, so Patchwork refuses to redefine anything the file defines (`Patchwork\Exceptions\DefinedTooEarly`). Do not delete the translation stubs on the assumption that Brain Monkey replaced them; that is a fatal undefined-function error the moment any code under test calls `__()`.
