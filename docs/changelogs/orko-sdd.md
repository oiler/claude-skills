...
- Follow-up flags Decided rows that lack the `Ruling: <what> — <why> — cost if wrong: <cost>` shape, supersedes notes that match nothing, plain rulings that read as reversals, and rulings an unindented line may continue. It also lists lines that look like rulings but didn't parse. Finish has the orchestrator fix those ledger lines and run `report` once more.
- The ruling parser keeps empty rulings, joins indented continuation lines, reads `**Ruling:**`, `**Ruling**:`, and table-cell rulings cleanly, ends an annotation at its own closing parenthesis, and reads `Final: parked` lines with their findings. A ruling quoted in a `Follow-up:` line is no longer a decision.
